...
const rdGame = (bd, id, tm) =>
	Object.keys(bd).find((a) => a.startsWith(id) && a.endsWith(tm));
const iGame = (bd, id) =>
	((a) => a && bd[a].id)(Object.keys(bd).find((a) => a.startsWith(id)));
const tGame = (bd, id) =>
	((a) => a && bd[a].time)(Object.keys(bd).find((a) => a.startsWith(id)));
...
const gameSlot = async (conn, m, db) => {
	...
	const listSlot1 = `${pickRandom(sotoy)} : ${pickRandom(
		sotoy
	)} : ${pickRandom(sotoy)}`;
	const listSlot2 = `${slot1} : ${slot2} : ${slot3}`;
	const listSlot3 = `${pickRandom(sotoy)} : ${pickRandom(
		sotoy
	)} : ${pickRandom(sotoy)}`;
	...
	try {
		if (slot1 === slot2 && slot2 === slot3) {
			...
			let sloth = `[  🎰VIRTUAL SLOT 🎰  ]\n------------------------\n\n${listSlot1}\n${listSlot2} <=====\n${listSlot3}\n\n------------------------\n[  🎰 VIRTUAL SLOT 🎰  ]\n\n*Keterangan* :\n_You Win🎉_ <=====Limit + ${randomLimit}, Uang + ${
				randomLimit * 500
			}`;
...
const gameCasinoSolo = async (conn, m, prefix, db) => {
	try {
		...
		if (db.users[m.sender].uang >= count * 1) {
			...
			m.reply(
				`Uang kamu tidak mencukupi untuk Casino silahkan *kumpulkan* terlebih dahulu!`
			);
		}
	} catch (e) {
		console.log(e);
...
const gameMerampok = async (m, db) => {
	...
	if (new Date() - db.users[m.sender].lastrampok > 3600000) {
		...
		if (10000 > db.users[who].uang)
			return m.reply("Targetnya Kismin ngab🗿");
	...
	if (new Date() - user.lastbegal > 3600000) {
		...
		if (teksnya.no === 0) {
			...
			await conn.sendMessage(
				m.chat,
				{ text: "Gagal Mencari Pemain, Silahkan Coba lagi" },
				{ quoted: m }
			);
			...
			await conn.sendMessage(
				m.chat,
				{ text: `Berhasil Mendapatkan Uang Sebesar : *${randomUang}*` },
				{ quoted: m }
			);
			db.users[m.sender].uang += randomUang;
		...
		conn.sendMessage(
			m.chat,
			{
				text: `Silahkan tunggu *⏱️${timers}* lagi untuk bisa bermain lagi`
			},
			{ quoted: m }
		);
	}
...
const buy = async (m, args, db) => {
	if (args[0] === "limit") {
		...
		if (db.users[m.sender].uang >= args[1] * 1) {
			...
			if (db.users[m.sender].uang <= args[1] * 1) {
				m.reply(" Duit lu ga cukup cik!");
				db.users[m.sender].uang += args[1] * 500;
			} else {
				db.users[m.sender].limit += args[1] * 1;
				m.reply(
					`Berhasil Membeli Limit Sebanyak ${
						args[1] * 1
					} dengan harga ${args[1] * 500}`
				);
			}
		} else {
			m.reply(
				`Uang Kamu Tidak Cukup Untuk Membeli limit!\nUangmu Tersisa : ${
					db.users[m.sender].uang
				}\nHarga ${args[1]} Limit : ${args[1] * 500}`
			);
		}
	} else {
		m.reply(
			`Harga Limit : Jumlah x 500\n• 1 limit = 500\n• 2 limit = 1000\n\nExample : .buy limit 3`
		);
	}
};
		...
		if (db.users[who]) {
			if (db.users[m.sender].limit >= count * 1) {
				try {
					...
					m.reply(
						`Berhasil mentransfer limit sebesar ${count}, kepada @${
							who.split("@")[0]
						}`
					);
				} catch (e) {
		...
		if (db.users[who]) {
			if (db.users[m.sender].uang >= count * 1) {
				try {
					...
					m.reply(
						`Berhasil mentransfer uang sebesar ${count}, kepada @${
							who.split("@")[0]
						}`
					);
				} catch (e) {
				...
				m.reply(
					`Uang tidak mencukupi!!\Uang mu tersisa : *${
						db.users[m.sender].uang
					}*`
				);
			}
